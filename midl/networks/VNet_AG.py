...
class VNet_AG(nn.Module):
    def __init__(self):
        super(VNet_AG, self).__init__()
        ...
        self.gating = GatingSignal(in_channels=256, out_channels=128)

        # Attention gates
        self.attention32 = AttentionGate(in_channels=32, gating_channels=128, inter_channels=16)
        self.attention64 = AttentionGate(in_channels=64, gating_channels=128, inter_channels=32)
        self.attention128 = AttentionGate(in_channels=128, gating_channels=128, inter_channels=64)
        self.attention256 = AttentionGate(in_channels=256, gating_channels=128, inter_channels=128)
    ...
    def forward(self, x):
        ...
        # gating
        gating = self.gating(out256)

        out32 = self.attention32(out32, gating)
        out64 = self.attention64(out64, gating)
        out128 = self.attention128(out128, gating)
        out256 = self.attention256(out256, gating)
